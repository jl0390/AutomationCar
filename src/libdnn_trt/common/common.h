...
{
    auto inds = trt::argsort(output.cbegin(), output.cend(), true);
...
inline std::vector<size_t> topK(const std::vector<T> inp, const size_t k)
{
    ...
    std::vector<size_t> inds = trt::argsort(inp.cbegin(), inp.cend(), true);
...
template <int C, int H, int W>
inline void readPPMFile(const std::string& filename, trt::PPM<C, H, W>& ppm)
{
